    ...
    def simulate_experiment(self):
        ...
        for b in range(self.nb):
            ...
            for t in range(self.nt + 1):
                ...
                if (t == 0) & (b==0):
                    self.agent.update_beliefs(states)
                else:
                    self.agent.update_beliefs(states, self.outcomes[:,b,t-1], self.responses[:, b, t-1])

                if t < self.nt:

                    res = self.agent.sample_responses()
